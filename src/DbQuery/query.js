...
export const queryPollingUnitResults = (res, id) => {
  const query = `SELECT * FROM bincom_inyang.announced_pu_results WHERE polling_unit_uniqueid = ${id}`;
...
export const queryPollingUnitTotalResults = (res, id) => {
  const query = `SELECT SUM(party_score) AS total FROM bincom_inyang.announced_pu_results WHERE polling_unit_uniqueid IN (SELECT polling_unit_id FROM polling_unit WHERE lga_id = ${id})`;
  database.query(query, (err, results) => {
    if (err) return res.status(400).json(err);
...
export const getAllLgas = (res) => {
  const query = "SELECT * FROM bincom_inyang.lga";
  database.query(query, (err, results) => {
    if (err) {
...
export const getAllParties = (res) => {
  const query = "SELECT * FROM bincom_inyang.party";
  database.query(query, (err, results) => {
    if (err) {
      return res.status(400).json(err);
    }
